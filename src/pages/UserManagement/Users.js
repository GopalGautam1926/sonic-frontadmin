      ...
      options: {
        customBodyRender: (value) => {
          ...
          if (
            ...
            rowData?.userRole === userRoles.PARTNER_USER
          ) {
            return rowData?.partner?.name;
          } else if (
            rowData?.userRole === userRoles.COMPANY_ADMIN ||
            rowData?.userRole === userRoles.COMPANY_USER
          ) {
      ...
      options: {
        customBodyRender: (value) => {
          if (value === userRoles.ADMIN) {
            ...
            value === userRoles.PARTNER_ADMIN ||
            value === userRoles.COMPANY_ADMIN
          ) {
            return "Admin";
          } else if (
            value === userRoles.PARTNER_USER ||
            value === userRoles.COMPANY_USER ||
            value === userRoles.PORTAL_USER
          ) {
            return "Standard";
          }
          return "---";
          ...
          if (!value) {
            statusItem.push(
              <Badge
                color="error"
                size="small"
                label={<div style={{ fontSize: 11 }}>Inactive</div>}
              />
            );
          }
          if (value) {
        ...
        customBodyRender: (value, { columnIndex }, updateValue) => {
          ...
          return (
            <Table.TableRowAction
              ...
              viewButtonProps={{
                onClick: () => {
                  const path = `${getRouteNames()["um_users"]}/view/${value}`;
                  history.push({
                    pathname: path,
                    state: {
                      user: rowData,
              ...
              title={
                <Table.TableActions
                  ...
                  onChangeStartDate={(date) =>
                    userStore?.changeDateRange({
                      ...userStore?.getDateRange,
                      startDate: date,
                    })
                  }
